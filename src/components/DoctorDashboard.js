...
import { HeartPulse, CheckSquare, Microscope, Eye, Beaker } from 'lucide-react';
...
export default function DoctorDashboard() {
  ...
  const returningFromLab = db.visits.filter(v =>
    v.diagnosis === "" &&
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border dark:border-slate-700 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Lab Tests Sent & Pending</span>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border dark:border-slate-700 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Back From Lab Tests</span>
            <p className="text-3xl font-bold text-indigo-600 dark:text-indigo-400 mt-1">{returningFromLab.length}</p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border dark:border-slate-700 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">My Completed Consults</span>
      ...
      {/* Patients coming back from laboratory tests */}
      <div className="bg-white dark:bg-slate-800 rounded-xl border-2 border-indigo-500/50 dark:border-indigo-500/30 shadow-md overflow-hidden">
        <div className="p-5 border-b dark:border-slate-700 bg-indigo-50/30 dark:bg-indigo-950/10 flex justify-between items-center">
          <div>
            <h3 className="font-bold text-indigo-950 dark:text-indigo-300">Patients Back from Lab (Ready for Review & Finalization)</h3>
            <p className="text-xs text-indigo-500 dark:text-indigo-400/80 mt-1">These patients have completed their laboratory tests and are waiting for you to review and finalize their consultation diagnosis.</p>
          </div>
          <span className="px-2.5 py-1 rounded-full bg-indigo-100 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-400 text-xs font-bold">{returningFromLab.length}</span>
        </div>
        <div className="overflow-x-auto w-full">
          {returningFromLab.length === 0 ? (
            <p className="p-8 text-center text-slate-500 text-sm">No patients returning from lab tests today.</p>
          ) : (
            <table className="w-full text-left text-sm border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-700/30 border-b dark:border-slate-700 text-slate-400 font-bold uppercase text-xs">
                  ...
                  <th className="p-4">Lab Tests Conducted</th>
                  <th className="p-4">Results Review</th>
                  <th className="p-4">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y dark:divide-slate-700">
                {returningFromLab.map(v => {
                  const p = db.patients.find(x => x.id === v.patientId);
                  return (
                    <tr key={v.visitNo} className="hover:bg-indigo-50/10 dark:hover:bg-indigo-950/5">
                      ...
                      <td className="p-4">
                        <div className="flex gap-1.5 flex-wrap">
                          {v.labResults.map((r, idx) => (
                            <span key={idx} className="px-2 py-0.5 rounded text-2xs font-bold bg-green-100 text-green-700 dark:bg-green-950/30 dark:text-green-400">
                              {r.testName}: {r.value}
                            </span>
                          ))}
                        </div>
